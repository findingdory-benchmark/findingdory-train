...
# Add your own run directory here
RUN_DIR="runs/jun_30"
...
if [ -n "$SPECIFIC_DIRS" ]; then
    ...
    # Set RUN_DIR to the debug directory
    RUN_DIR=$DEBUG_DIR
    # Process only the root directory itself
    DIRS_TO_PROCESS=("$RUN_DIR")
else
    # Normal mode - process all directories in RUN_DIR
    DIRS_TO_PROCESS=($RUN_DIR/*)
fi
...
#SBATCH --output=slurm_logs/evals/${MODEL_NAME}/${CKPT_ID}-%j.out
#SBATCH --error=slurm_logs/evals/${MODEL_NAME}/${CKPT_ID}-%j.err
#SBATCH --gpus=a40:1
#SBATCH --nodes=1
